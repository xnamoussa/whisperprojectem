...
def _build_base_dataframe(ministry: str, size: int = 500) -> pd.DataFrame:
    ...
    modes = ["Bus", "Tramway", "Métro", "RER", "Velo", "Covoiturage"]

    df = pd.DataFrame(
        {
            "ridership": rng.normal(350, 80, size), # Global mobility is higher in FR
            "budget": rng.normal(450, 100, size),
            "staff": rng.integers(50, 250, size),
            "accidents": rng.poisson(2, size),
            "pollution_index": rng.normal(42, 12, size),
            "satisfaction": rng.uniform(3.0, 4.8, size).round(2),
            "region": rng.choice(["Île-de-France", "AURA", "PACA", "Occitanie", "Hauts-de-France", "Grand-Est"], size=size),
            "city": rng.choice(cities, size=size),
            "transport_mode": rng.choice(modes, size=size),
        }
    )
    signal = (
        0.35 * df["ridership"]
        ...
        + 0.08 * df["staff"]
        - 0.12 * df["accidents"]
        + 0.05 * df["satisfaction"]
    )
    df["target_class"] = (signal > np.percentile(signal, 52)).astype(int)
    df["target_reg"] = signal + rng.normal(0, 20, size)

    # Inject missing values and outliers to demonstrate data cleaning
    missing_idx = rng.choice(size, size // 12, replace=False)
    outlier_idx = rng.choice(size, size // 18, replace=False)
    df.loc[missing_idx, "budget"] = np.nan
    df.loc[rng.choice(size, size // 20, replace=False), "satisfaction"] = np.nan
    df.loc[outlier_idx, "ridership"] *= 2.5
    return df
...
def _clean_and_engineer(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    cleaned["budget_per_staff"] = cleaned["budget"] / cleaned["staff"]
    cleaned["service_intensity"] = cleaned["ridership"] / cleaned["staff"]
...
# C — Classification
# ---------------------------------------------------------------------------
def _classification(df: pd.DataFrame) -> dict[str, Any]:
    ...
    candidates = [
        ...
        (
            "RandomForestClassifier",
            Pipeline([("prep", prep), ("model", RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42))]),
        ),
        (
            "GradientBoosting",
            Pipeline([("prep", prep), ("model", GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42))]),
        ),
    ]
    ...
    for name, estimator in candidates:
        ...
        if f1 > best_f1:
            ...
            if hasattr(inner, "feature_importances_"):
                ohe = estimator.named_steps["prep"].named_transformers_["cat"].named_steps["ohe"]
...
# D — Regression
# ---------------------------------------------------------------------------
def _regression(df: pd.DataFrame) -> dict[str, Any]:
    prep, numeric_cols, _ = _preprocessor(df, ["target_class", "target_reg"])
    x = df.drop(columns=["target_class", "target_reg"])
    y = df["target_reg"]
    x_train, x_test, y_train, y_test = train_test_split(
...
# E — Clustering
# ---------------------------------------------------------------------------
def _clustering(df: pd.DataFrame) -> dict[str, Any]:
    numeric = df[
        ["ridership", "budget", "staff", "budget_per_staff", "service_intensity", "accidents", "pollution_index"]
    ].copy()
    ...
    gmm = GaussianMixture(n_components=3, random_state=42).fit(scaled)
    ...
    for k in range(2, 7):
        ...
        silhouette_data.append(
            {"k": k, "silhouette": round(float(silhouette_score(scaled, km.labels_)), 4)}
        )
    ...
    for name, labels in [
        ...
        if len(unique) < 2:
            ...
            dbi = round(float(davies_bouldin_score(scaled, labels)), 4)
    ...
    pca_scatter = [
        ...
        for i in range(0, len(pca_coords), max(1, len(pca_coords) // 150))
    ]

    # Heatmap data (cluster means)
    heatmap_cols = ["ridership", "budget", "staff", "pollution_index", "accidents"]
    heatmap_data = (
        pd.DataFrame(numeric[heatmap_cols])
        .assign(cluster=kmeans.labels_)
        .groupby("cluster")
        .mean()
        .round(2)
    ...
    heatmap = {
        ...
        "rows": [
            {"cluster": int(idx), "values": row.tolist()}
...
# ---------------------------------------------------------------------------
def _time_series(ministry: str) -> dict[str, Any]:
    ...
    try:
        ...
        kpss_result = [0, 1] # Fallback to stationary-like
...
# Main entry point
# ---------------------------------------------------------------------------
def run_ministry_ml_report(ministry: str) -> dict[str, Any]:
    base_df = _build_base_dataframe(ministry=ministry)
    prepared_df, prep_summary = _clean_and_engineer(base_df)
    
    try:
        classification = _classification(prepared_df)
    except Exception:
        classification = {"models": [], "roc_curves": []}
        
    try:
        regression = _regression(prepared_df)
    except Exception:
        regression = {"models": [], "residuals": []}
        
    try:
        clustering = _clustering(prepared_df)
    except Exception:
        clustering = {"models": [], "elbow": []}
        
    try:
        forecasting = _time_series(ministry=ministry)
    except Exception:
    ...
    result = {
        ...
        "model_understanding": {
            "global_assumptions": [
                ...
                "Regression assumes approximate linearity or tree-based non-linear capture",
                "Time-series assumes temporal dependence and seasonal monthly patterns",
                "Clustering assumes meaningful separation in feature space",
            ],
            "limitations": [
                "Training data is generated per ministry seed (production would use real data)",
                "Prophet and LSTM integrations require additional runtime packages",
                "Cross-validation scores may differ from test set performance",
            ],
        },
    }
