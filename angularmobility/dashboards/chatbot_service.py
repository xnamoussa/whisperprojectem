    ...
    def process_message(message: str, ministry: str, user_name: str) -> str:
        ...
        if api_key and api_key != "YOUR_REAL_API_KEY_HERE":
            try:
                ...
                prompt = f"Tu es l'assistant IA SASA du Ministère {ministry} en France. Réponds à la question suivante de manière courte et experte: {message}"
                payload = {
        ...
        prefix = context_prompts.get(ministry.lower(), "Assistant SASA-ML,")
        ...
        if any(word in msg for word in ["bonjour", "salut", "hello"]):
            return f"Bonjour Monsieur le Ministre {user_name}. Je suis l'assistant SASA spécialisé pour le département {ministry}. Comment puis-je vous éclairer sur les données de mobilité aujourd'hui ?"
...
chatbot_service = SASAChatbotService()
